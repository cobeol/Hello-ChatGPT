...
# proxies 없이 http_client 사용 (proxies= 가 에러를 일으킴)
client = AsyncOpenAI(
...
async def get_chatgpt(
    ...
    try:
        ...
        if "maximum context length" in str(e):
            raise Exception("너무 긴 답변을 유도하셨습니다.")
        else:
            raise Exception("오류가 발생했습니다. 다시 시도해 주세요.")
    except APITimeoutError:
        raise Exception("OpenAI 서버 응답 없음. 다시 시도해 주세요.")
            ...
            if gpt_model in ("gpt-4-turbo", "gpt-4o") and (files := history.get("files", [])):
                for file in files:
                    ...
                    if download_file(url, filename):
                        content.append({
                            "type": "image_url",
                            "image_url": {
                                "url": f"data:{file.get('mimetype')};base64,{encode_image(filename)}"
                            }
                        })
                    else:
                ...
                if index == len(chat_history):
                    if list(filter(lambda x: x["size"] > MAX_FILE_BYTES, files)):
                        raise Exception(f"{MAX_FILE_BYTES/1_000_000}MB 이상 이미지는 처리 불가")

            content.append({
                "type": "text",
                "text": re.sub(r"<@(.*?)>", "", history.get("text"))
            })
